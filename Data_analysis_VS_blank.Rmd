...
Saccades %>% 
  ...
  ggplot(aes(x=Amplitude)) +
  geom_density()

Saccades %>% 
  ggplot(aes(x=Amplitude, fill=Task)) +
...
```{r}
mean(na.omit(Saccades$Amplitude) )

Saccades %>% 
  remove_missing() %>% 
  ggplot(aes(y=Amplitude, x=Task, fill=Task)) +
  geom_boxplot()
```
...
mGaus <- lmer(Amplitude ~ Task + (1 + Trial | ParticipantID), data=Saccades)

mLog <- glmer(Amplitude ~ Task + (1 + Trial | ParticipantID), family=gaussian( link="log"), data=Saccades)
...
plot(density(na.omit(Saccades$Amplitude)), main = "Observed amplitude")
```
```{r}
anova(mGaus, mLog)
...
```{r}
SumSaccades <- Saccades %>% 
  group_by(ParticipantID, Task, Trial) %>% 
  summarise(SumAmp=sum(Amplitude))

hist(SumSaccades$SumAmp)

mGausSum <- lmer(SumAmp ~ Task + (1 + Trial | ParticipantID), data=SumSaccades)

mGausSum1 <- lmer(SumAmp ~ Task + (1 + Trial:Task | ParticipantID), data=SumSaccades)

mGausSum2 <- lmer(SumAmp ~ Task + (1 | ParticipantID), data=SumSaccades)



anova(mGausSum, mGausSum1, mGausSum2)
...
Fixations %>% 
  ggplot(aes(x=Duration, fill=Task)) +
  geom_density(alpha = 0.5) +
  xlim(0, 1000)
```
...
mGaus <- lmer(Duration ~ Task + (1 + Trial | ParticipantID), data=Fixations)

mLog <- glmer(Duration ~ Task + (1 + Trial | ParticipantID), family=gaussian( link="log"), data=Fixations)
